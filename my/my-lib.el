...
;; Last Updated: <2022/07/07 20:00:23>
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;;                自 作 関 数 集
...
(defun my/close-all-buffers ()
   ...
   (dolist (b (buffer-list))
     (if (or (not (buffer-file-name b)) (and (buffer-file-name b) (not (buffer-modified-p b))))
         (kill-buffer b))))

;;開いているファイルバッファで変更してないものを全部閉じる
(defun my/close-all-buffers-file ()
   (interactive)
   (dolist (b (buffer-list))
     (if (and (buffer-file-name b) (not (buffer-modified-p b)))
         (kill-buffer b))))

;;開いているバッファでファイル編集ではないものを全部閉じる
(defun my/close-all-buffers-no-file ()
   (interactive)
   (dolist (b (buffer-list))
     (if (not (buffer-file-name b))
         (kill-buffer b))))
